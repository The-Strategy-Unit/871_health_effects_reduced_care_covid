---
title: "Effects of physical violence diagnoses table using reactable"
...
# ED
ed_dat <- ed_grp |>
  filter(grpnm == "Effects of physical violence") |> 
  select(diagnm, p2, u) |>
  arrange(desc(u))
...
short_nms <- tribble(
  ~ icd10, ~ shortnm,
  "R930", "Abnormal findings imaging of head, nec."
  )
...
react_tbl_div(tbl, selector = "rtbl-grp-ed", title = "ED diagnoses for effects of physical violence")
```

```{r, react-tbl-ip, echo=FALSE}
tbl <- build_react_tbl(ip_dat, units = "Admis.", colwidth = c(240, 200, 200))

react_tbl_div(tbl, selector = "rtbl-grp-ip", title = "Inpatient diagnoses for effects of physical violence")
```
