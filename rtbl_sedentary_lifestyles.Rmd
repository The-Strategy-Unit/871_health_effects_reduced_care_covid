---
title: "Sedentary lifestyles diagnoses table using reactable"
...
# ED
ed_dat <- ed_grp |>
  filter(grpnm == "Sedentary lifestyles") |> 
  select(diagnm, p2, u) |>
  arrange(desc(u))
...
react_tbl_div(tbl, selector = "rtbl-grp-ed", title = "ED diagnoses for spinal/back conditions")
```

```{r, react-tbl-ip, echo=FALSE}
tbl <- build_react_tbl(ip_dat, units = "Admis.", colwidth = c(240, 200, 200))

react_tbl_div(tbl, selector = "rtbl-grp-ip", title = "Inpatient diagnoses for spinal/back conditions")
```
